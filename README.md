...
| もの | 用途 | 補足 |
|------|------|------|
| **AI** | アシスタントを書く・検証を回す | 2種類のどちらでも作れます。**手元の PC で動く AI**（このフォルダを直接読める。Claude Code など）と、**Web 上で動く AI**（手元のフォルダは見えないので ZIP で渡す。ChatGPT など）。動作確認済みは Claude Code と ChatGPT（有料版）。無料版のチャット AI は対象外です（確認しているのは有料版だけです） |
...
| **PHP 7.4 以上** | 検証コマンド `validate.php` を動かす | **手元の PC で動く AI に検証させるときだけ**。Web 上で動く AI に検証させる場合は不要（AI 側のサーバーで動きます） |
...
**A. 手元の PC で動く AI（Claude Code など）**
...
**B. Web 上で動く AI（ChatGPT など）**

1. 2 で製品のコードを置いた状態のリポジトリのフォルダを、**フォルダごと1つの ZIP** にする（展開後 20MB 前後・圧縮すると 7MB 程度・1,300 ファイルほど）。製品のコードが同じ ZIP に入っていないと検証が動きません。
2. 新しいチャットにその ZIP を添付し、次の文と一緒に「作ってほしいもの」を送る。Web 上で動く AI は手元の `AGENTS.md` を自動では読まないので、この文の1行目がその代わりです。
