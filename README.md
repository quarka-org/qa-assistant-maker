...
**ChatGPT（有料版・Web）でも作れます。** このフォルダを、**製品のコードを置いた状態のまま1つの ZIP にして添付**し、「**`AGENTS.md` に従って作って。できあがった3ファイルは個別に出して**」と伝えてください。ZIP の中を読み、検証コマンドも自分で実行できることを 2026-09 に確認しています。
...
3. リポジトリ直下で **AI エージェントを起動する**（多くの AI は `AGENTS.md` を自動で読み込みます。Claude Code は `CLAUDE.md` 経由で同じものを読みます。自動で読まない AI には「`AGENTS.md` を読んでから作って」と伝えてください）
